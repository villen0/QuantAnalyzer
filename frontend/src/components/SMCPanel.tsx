...
const structureColor = (s: string) =>
  s === 'bullish' ? '#10b981' : s === 'bearish' ? '#ef4444' : '#9ca3af';
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
        {/* Market Structure */}
        <div style={{ background: '#f8fafc', borderRadius: 8, border: '1px solid #e5e7eb', padding: '10px 12px', textAlign: 'center' }}>
          <div style={{ fontSize: 9, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
            Market Structure
          </div>
          <Badge
        ...
        <div style={{ background: '#f8fafc', borderRadius: 8, border: '1px solid #e5e7eb', padding: '10px 12px', textAlign: 'center' }}>
          ...
          {smc.choch ? (
            <>
              ...
              />
              <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 6 }}>${smc.choch.level.toFixed(2)}</div>
            </>
          ) : smc.bos ? (
            <>
              <Badge
              ...
              />
              <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 6 }}>${smc.bos.level.toFixed(2)}</div>
            </>
          ) : (
            <>
              <Badge label="NO BREAK" color="#6b7280" bg="rgba(107,114,128,0.1)" />
              <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 6 }}>Watching for break</div>
            </>
          )}
        </div>
        ...
        <div style={{ background: '#f8fafc', borderRadius: 8, border: '1px solid #e5e7eb', padding: '10px 12px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 6 }}>EQ: ${pd.equilibrium.toFixed(2)}</div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        {smc.order_blocks.bullish.every(o => o.mitigated) && smc.order_blocks.bearish.every(o => o.mitigated) && (
          <div style={{ fontSize: 12, color: '#9ca3af', padding: '4px 0' }}>No active order blocks detected</div>
        )}
        {smc.order_blocks.bullish.map((ob, i) => (
          <div key={`bull-ob-${i}`} style={{
            ...
            border: `1px solid ${ob.mitigated ? '#e5e7eb' : '#10b98133'}`,
            opacity: ob.mitigated ? 0.5 : 1,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: '#10b981' }}>▲ Bullish OB</span>
              <span style={{ fontSize: 10, color: '#9ca3af' }}>${ob.low.toFixed(2)} – ${ob.high.toFixed(2)}</span>
            </div>
            <span style={{ fontSize: 10, color: ob.mitigated ? '#9ca3af' : '#10b981', fontWeight: 600 }}>
        ...
        {smc.order_blocks.bearish.map((ob, i) => (
          <div key={`bear-ob-${i}`} style={{
            ...
            border: `1px solid ${ob.mitigated ? '#e5e7eb' : '#ef444433'}`,
            opacity: ob.mitigated ? 0.5 : 1,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 10, color: ob.mitigated ? '#9ca3af' : '#ef4444', fontWeight: 600 }}>
              {ob.mitigated ? 'Mitigated' : currentPrice >= ob.low && currentPrice <= ob.high ? '⚡ In OB' : 'Active'}
            </span>
          </div>
        ))}
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        {smc.fair_value_gaps.length === 0 && (
          <div style={{ fontSize: 12, color: '#9ca3af', padding: '4px 0' }}>No recent FVGs detected</div>
        )}
        {smc.fair_value_gaps.map((fvg, i) => {
          ...
          return (
            <div key={`fvg-${i}`} style={{
              ...
              border: `1px solid ${fvg.mitigated ? '#e5e7eb' : col + '33'}`,
              opacity: fvg.mitigated ? 0.5 : 1,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: col }}>{isBull ? '▲' : '▼'} {isBull ? 'Bullish' : 'Bearish'} FVG</span>
                <span style={{ fontSize: 10, color: '#9ca3af' }}>${fvg.bottom.toFixed(2)} – ${fvg.top.toFixed(2)}</span>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 10, color: fvg.mitigated ? '#9ca3af' : col, fontWeight: 600 }}>
                  {fvg.mitigated ? 'Filled' : 'Open'}
                </div>
                <div style={{ fontSize: 9, color: '#9ca3af' }}>gap ${fvg.size.toFixed(2)}</div>
              </div>
            </div>
          );
        })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div>
            ...
            {smc.liquidity.sell_side.length === 0
              ? <div style={{ fontSize: 11, color: '#9ca3af' }}>None identified</div>
